; Captured verbatim from CUDA-Q's qir-adaptive braket pipeline (LLVM 16).
; Source kernel:
;   __qpu__ void cswap() {
;     cudaq::qubit a, b, c;
;     h(a); x(b);
;     swap<cudaq::ctrl>(a, b, c);
;     mz(a); mz(b); mz(c);
;   }
; With `basis=h,s,t,rx,ry,rz,x,y,z,swap,x(1)` in braket.yml, controlled-SWAP
; (Fredkin) is *not* decomposed by the basis-gate pass - `swap` is in the
; basis but `swap(1)` (one-control swap) is not - so it reaches qirtoqasm
; as a variadic `generalizedInvokeWithRotationsControlsTargets` call.
...
define void @__nvqpp__mlirgen__function_cswap._Z5cswapv() local_unnamed_addr #0 {
"0":
  tail call void @__quantum__qis__h__body(ptr null)
  tail call void @__quantum__qis__x__body(ptr nonnull inttoptr (i64 1 to ptr))
  tail call void (i64, i64, i64, i64, ptr, ...) @generalizedInvokeWithRotationsControlsTargets(i64 0, i64 0, i64 1, i64 2, ptr nonnull @__quantum__qis__swap__ctl, ptr null, ptr nonnull inttoptr (i64 1 to ptr), ptr nonnull inttoptr (i64 2 to ptr))
...
declare void @__quantum__qis__swap__ctl(ptr, ptr, ptr)
