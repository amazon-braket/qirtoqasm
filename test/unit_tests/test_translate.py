...
class TestPackageSurface:
    def test_translate_is_callable(self) -> None:
    ...
    def test_dunder_all_lists_only_public_surface(self) -> None:
...
# ---------------------------------------------------------------------------
# translate() on representative programs
# ---------------------------------------------------------------------------


class TestTranslateBellState:
    """Canonical Bell-state program — the smallest useful end-to-end check."""

    def test_header_is_openqasm_3_0(self, translate) -> None:
        assert translate(BELL_IR).startswith("OPENQASM 3.0;")

    def test_quantum_and_classical_register_declarations(self, translate) -> None:
        out = translate(BELL_IR)
        assert "qubit[2] q;" in out
        assert "bit[2] c;" in out

    def test_gate_sequence(self, translate) -> None:
        out = translate(BELL_IR)
        assert "h q[0];" in out
...
class TestBraketTargetedEmitChoices:
    """The translator exists to feed Braket directly; these invariants
    are what make that work. Regressions here break Braket compatibility.
    """

    def test_cnot_emitted_even_when_qir_uses_cx(self, translate) -> None:
    ...
    def test_ccnot_emitted_for_toffoli(self, translate) -> None:
        ir = """
        %Qubit = type opaque
    ...
    def test_ising_rotations_emit_braket_aliases(
        self, translate, qir_name: str, emit_name: str
    ) -> None:
    ...
    def test_classical_register_is_plain_bit_not_output(self, translate) -> None:
    ...
    def test_no_stdgates_include_emitted(self, translate) -> None:
        out = translate(BELL_IR)
        assert "include" not in out


class TestAdaptiveProfileMidCircuitMeasurement:
    """Mid-circuit measurement with classical feedforward — the
    headline Adaptive-profile feature.
    """

    def test_if_only_branch_on_read_result(self, translate) -> None:
        ir = """
        %Qubit = type opaque
        %Result = type opaque
    ...
    def test_if_else_branch_emits_both_arms(self, translate) -> None:
        ir = """
        %Qubit = type opaque
        %Result = type opaque
...
class TestParametricRotations:
    """Rotation gates with scalar arguments hit the argument/qubit
    separation logic in the gate builder.
    """

    @pytest.mark.parametrize("gate", ["rx", "ry", "rz"])
    def test_single_qubit_rotation(self, translate, gate: str) -> None:
        ir = f"""
        %Qubit = type opaque
    ...
    def test_adjoint_gate_uses_inv_modifier(self, translate) -> None:
...
# ---------------------------------------------------------------------------
# translate() error surface
# ---------------------------------------------------------------------------


class TestErrorSurface:
    """Every unsupported / malformed input must raise ``QirToQasmError``
    with a message that points at the root cause. No Python traceback
    spam, no silent failures.
    """

    def test_malformed_qir_raises_qir_to_qasm_error(self) -> None:
        with pytest.raises(QirToQasmError):
            qirtoqasm.translate("not valid qir at all")

    def test_empty_input_raises_qir_to_qasm_error(self) -> None:
        with pytest.raises(QirToQasmError):
            qirtoqasm.translate("")

    def test_module_without_entry_point_names_the_problem(self) -> None:
        ir = """
    ...
    def test_unknown_qis_intrinsic_names_the_callee(self) -> None:
        ir = """
        %Qubit = type opaque
    ...
    def test_unsupported_opcode_names_the_opcode(self) -> None:
    ...
    def test_unmapped_variadic_controlled_gate_names_intrinsic(self) -> None:
    ...
    def test_input_type_error_surfaces_from_pyo3(self) -> None:
...
class TestOutputInvariants:
    """Properties that must hold of every translated program."""
...
# ---------------------------------------------------------------------------
# Trailing `// generated-by:` comment + optional ``producer`` kwarg
# ---------------------------------------------------------------------------
...
class TestGeneratedByComment:
    def test_trailing_comment_always_present(self) -> None:
        last = _generated_by_line(qirtoqasm.translate(BELL_IR))
        assert last.startswith('// generated-by: {"name":"qirtoqasm",')

    def test_comment_includes_version_and_profile_fields(self) -> None:
        last = _generated_by_line(qirtoqasm.translate(BELL_IR))
        assert f'"version":"{qirtoqasm.__version__}"' in last or '"version":"' in last
        assert '"profile":"base_profile"' in last

    def test_producer_kwarg_surfaces_in_comment(self) -> None:
        last = _generated_by_line(qirtoqasm.translate(BELL_IR, producer="mylib 0.1.2"))
        assert '"producer":"mylib 0.1.2"' in last

    def test_producer_omitted_by_default(self) -> None:
        last = _generated_by_line(qirtoqasm.translate(BELL_IR))
        assert '"producer"' not in last

    def test_producer_none_omits_field(self) -> None:
        last = _generated_by_line(qirtoqasm.translate(BELL_IR, producer=None))
        assert '"producer"' not in last

    def test_producer_empty_string_omits_field(self) -> None:
    ...
    def test_producer_with_special_characters_is_json_escaped(self) -> None:
    ...
    def test_producer_with_newline_stays_on_one_line(self) -> None:
